...
void ofApp::draw() {
	...
	float size = 30;
	ofColor color;
	for (float x = size * 0.5; x <= ofGetWindowWidth() - size * 0.5; x += size) {

		for (float y = size * 0.5; y <= ofGetWindowHeight() - size * 0.5; y += size) {

			int deg = ofMap(ofNoise(x * 0.003, y * 0.003 + ofGetFrameNum() * 0.02), 0, 1, 0, 360);
			int hue = ofMap(deg, 0, 360, 100, 255);
			color.setHsb(hue, 180, 255);

			this->draw_arrow(glm::vec2(x, y), glm::vec2(x + cos(deg * DEG_TO_RAD), y + sin(deg * DEG_TO_RAD)), size * 0.5, ofColor(0), color);
		}
	}
}
